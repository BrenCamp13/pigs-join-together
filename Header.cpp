...
int rules::correct(int row, const Board& b)
{
	return 0; //need to find a way to correct invalid num selection to correct vertical pos
}


bool rules::answer_parser(const Board& b, string* ans, int* row, int* column, bool* quit)
{
...
bool rules::check_if_trio(int row, int column, Board & b, char player)
...
bool players::Black::move_piece(Board& b, int column, int row)
...
bool players::White::move_piece(Board& b, int column, int row)
