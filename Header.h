...
public:
	class White {
		bool move_piece(Board& b, int column, int row);
	};
	class Black {
		bool move_piece(Board& b, int column, int row);
	};
...
public:
	...
	int correct(int row, const Board& b); // convert the number to the correct place uppon the y axis

	bool answer_parser(const Board& b, string* ans, int* row, int* column, bool* quit);
